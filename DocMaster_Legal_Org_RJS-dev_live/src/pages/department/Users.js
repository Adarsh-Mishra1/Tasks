...
const Users = () => {
  ...
  const getAllDepartmentUsers = () => {
    ...
    axios
      .get(
        WsGetOrgUserDepartmentMap + "/" + userData.org.id + "/" + userData.id,
        {
  ...
  const processDataTable = (simpleDataArray) => {
    simpleDataArray.map((simpleData, index) => {
      ...
      simpleData["usrname"] = simpleData.user.name;
        ...
        <>
          {userData.crudAccess?.user.d == 1 ? (
            <i
              ...
              onClick={() => onOrgUserDepartmentDelete(simpleData)}
              aria-hidden="true"
            ></i>
          ) : null}
        </>
  ...
  const onOrgUserDepartmentDelete = (record) => {
    confirmAlert({
      title: "Confirm!",
      message:
        "Sure to remove User from '" + record.department.name + "' department",
      buttons: [
        {
          label: "Yes",
          onClick: () => {
