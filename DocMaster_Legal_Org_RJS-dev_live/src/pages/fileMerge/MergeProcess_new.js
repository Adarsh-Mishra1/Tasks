...
const MergeProcess = () => {
 ...
 const onSubmitHandler = async (e) => {
  ...
  // Fetch research statistics before proceeding with title and form validation
  try {
    /*
    ...
    // console.log("sri", data.resultMessage.researches_total, data.resultMessage.researches);

    // // Check if the user can proceed with the case creation
    // if (data.resultMessage.researches > data.resultMessage.researches_total) {
    */
    ...
    // if (!courtname || !location || !state || !complaintnumber || !complainant || !accused || !advocate || !date) {
    ...
    // Create a header data string
    const dataString = `${courtname}, ${location}, ${state}, ${complaintnumber}, ${complainant}, ${accused}, ${advocate}, ${formattedDate}`;

    // Update the header data state
    setHeaderData(dataString);

    // Proceed with file merging and record creation
    putFileMergeUserRecord(); // Call your function to submit the user record
...
// putFileMergeUserRecord function to handle the API request
function putFileMergeUserRecord() {
  alert("enter this method");

  // Ensure all required data is available before proceeding
  if (!userData || !userData.id || !userData.org.id || !mergeTemplate.id || !recordId || !title || !formData) {
    alert("Missing necessary information for submission");
    return; // Prevent submission if data is incomplete
  }

  // Sending the request to the server with Axios
  axios
    .post(
      WsPutFileMergeUserRecord,
      JSON.stringify({
      ...
      {
        headers: apiKeyHeader(), // Ensure you have the necessary API headers here
      }
    )
    .then((response) => {
      const responseData = response.data;
            ...
            <div className="page-title">
              <div className="title_left">
                <h6>
                  ...
                  >
                    Go Back ⬅️ &nbsp; Merge & Create Signle PDF for Online Filing 
                  </span>
                </h6>
              </div>
            </div>
                  ...
                  <>
                    <Form onSubmit={onSubmitHandler} method="POST">
                      ...
                      <FormGroup className="mb-3" row>
                        <Label for="inputTitle" className="col-md-3">
                                      ...
                                      <>
                                        <a
                                          ...
                                          href={
                                            isDisabled
                                              ? "#"
                                              : alreadyExist.fileURL
                                          }
                                          rel="noreferrer"
                                          onClick={handleClick}
                                        >
                                          <i>
                                            <p>{alreadyExist.originalName}</p>
                                          </i>
                                          <i
