...
import MasterOrg1 from "./taskOrganiser/masterOrg";
import {
...
function CaseDiaryCalendar() {
  ...
  const { error, loader } = useSelector((state) => state.caseDiaryOrg);
  ...
  useEffect(() => {
    dispatch(
      GetCalendarDataThunk({ userId: userData.id, orgId: userData.org.id })
    );
  }, []);
  ...
  const handleCloseScheduleTask = (value) => {
    dispatch(handleTaskModal(value));
  };

  if (error?.caseDiaryError) {
    alert(`${error.caseDiaryError}`);
  }
  ...
  return (
    <Suspense fallback={<>Loading...</>}>
      <div className="main_container">
        <Sidebar />
        <Navbar />
        {loader.caseDiaryLoader && <LoadingSpinner />}
        ...
        <div className="right_col " role="main" style={{ overflow: "auto" }}>
          ...
          <div className="mt-2">
            ...
            <Button
              title="Calendar view"
              backgroundColor={
                currentDisplayComponent === "calendar" ? "#0d6efd" : "white"
              }
              onClick={() => {
                dispatch(handleCurrentDisplayComponent("calendar"));
              }}
            />
            <Button
              title="Grid view"
              backgroundColor={
                currentDisplayComponent === "grid" ? "#0d6efd" : "white"
              }
              onClick={() => {
                dispatch(handleCurrentDisplayComponent("grid"));
              }}
            />
            <Button title="Schedule Report" backgroundColor="white" />
            <Button title="Schedule Alert" backgroundColor="white" />
            <Button
              title="Master Organiser"
              backgroundColor="white"
              onClick={() => {
                dispatch(handleMasterOrgModal(true));
              }}
            />
            <Button title="Schedule Task" backgroundColor="white" />
          </div>
          <div className="mt-2">
            {currentDisplayComponent === "grid" && <GridView events={events} />}
            {currentDisplayComponent === "calendar" && <CustomCalendar />}
            {showMasterOrg && <MasterOrg1 />}
            {showCaseDiary && <EventView />}
            {showScheduledTask && (
              <Popup
                title={"Schedule Task"}
                minHeight="400px"
...
function Button({ title, backgroundColor, onClick }) {
