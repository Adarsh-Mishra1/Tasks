...
describe("flot touch navigate plugin", function () {
  ...
  describe("touchZoom", function () {
    it("should zoom the plot", function () {
      ...
      var eventHolder = plot.getEventHolder(),
        ...
        midPointCoords = {
          x:
            (xaxis.c2p(finalCoords[0].x - plot.offset().left) +
              xaxis.c2p(finalCoords[1].x - plot.offset().left)) /
            2,
          y:
            (yaxis.c2p(finalCoords[0].y - plot.offset().top) +
              yaxis.c2p(finalCoords[1].y - plot.offset().top)) /
            2,
        },
      ...
      expect(xaxis.min).toBeCloseTo(
        (midPointCoords.x - initialXmin) * (1 - 1 / amount) + initialXmin,
        6,
      );
      expect(xaxis.max).toBeCloseTo(
        initialXmax - (initialXmax - midPointCoords.x) * (1 - 1 / amount),
        6,
      );
      expect(yaxis.min).toBeCloseTo(
        (midPointCoords.y - initialYmin) * (1 - 1 / amount) + initialYmin,
        6,
      );
      expect(yaxis.max).toBeCloseTo(
        initialYmax - (initialYmax - midPointCoords.y) * (1 - 1 / amount),
        6,
      );
    });
    ...
    it("should pan the plot for two fingers with constant distance", function () {
      ...
      expect(xaxis.min).toBeCloseTo(
        initialXmin + (canvasCoords[0].x - canvasCoords[2].x),
        6,
      );
      expect(xaxis.max).toBeCloseTo(
        initialXmax + (canvasCoords[0].x - canvasCoords[2].x),
        6,
      );
      expect(yaxis.min).toBeCloseTo(
        initialYmin + (canvasCoords[0].y - canvasCoords[2].y),
        6,
      );
      expect(yaxis.max).toBeCloseTo(
        initialYmax + (canvasCoords[0].y - canvasCoords[2].y),
        6,
      );
    });
    ...
    it("should zoom the plot on axis y", function () {
      ...
      expect(
        Math.abs(
          yaxis.max -
            (initialYmax - (initialYmax - midPointCoords.x) * (1 - 1 / amount)),
        ),
      ).toBeLessThan(1);
    });
  ...
  describe("touchDrag", function () {
    it("should drag the plot", function () {
      ...
      expect(xaxis.min).toBeCloseTo(
        initialXmin + (canvasCoords[0].x - canvasCoords[1].x),
        6,
      );
      expect(xaxis.max).toBeCloseTo(
        initialXmax + (canvasCoords[0].x - canvasCoords[1].x),
        6,
      );
      expect(yaxis.min).toBeCloseTo(
        initialYmin + (canvasCoords[0].y - canvasCoords[1].y),
        6,
      );
      expect(yaxis.max).toBeCloseTo(
        initialYmax + (canvasCoords[0].y - canvasCoords[1].y),
        6,
      );
    });
    ...
    it("should drag the logarithmic plot", function () {
      ...
      expect(yaxis.min).toBeCloseTo(
        yaxis.c2p(
          yaxis.p2c(initialYmin) + (pointCoords[0].y - pointCoords[1].y),
        ),
        6,
      );
      expect(yaxis.max).toBeCloseTo(
        yaxis.c2p(
          yaxis.p2c(initialYmax) + (pointCoords[0].y - pointCoords[1].y),
        ),
        6,
      );
    });
    ...
    it("should drag the point in the same way for many sequential moves as for one long move", function () {
      ...
      expect(
        Math.abs(
          xaxis.min -
            (initialXmin + (canvasCoords[1].x - canvasCoords[limit].x)),
        ),
      ).toBeLessThan(1);
      expect(
        Math.abs(
          xaxis.max -
            (initialXmax + (canvasCoords[1].x - canvasCoords[limit].x)),
        ),
      ).toBeLessThan(1);
      expect(yaxis.min).toBeCloseTo(
        initialYmin + (canvasCoords[1].y - canvasCoords[limit].y),
        0,
      );
      expect(yaxis.max).toBeCloseTo(
        initialYmax + (canvasCoords[1].y - canvasCoords[limit].y),
        0,
      );
    });
    ...
    it("should drag the plot on the xaxis only", function () {
      ...
      expect(xaxis.min).toBeCloseTo(
        xaxis.c2p(
          xaxis.p2c(initialXmin) + (pointCoords[0].x - pointCoords[1].x),
        ),
        6,
      );
      expect(xaxis.max).toBeCloseTo(
        xaxis.c2p(
          xaxis.p2c(initialXmax) + (pointCoords[0].x - pointCoords[1].x),
        ),
        6,
      );
      expect(yaxis.min).toBe(0);
    ...
    it("should recompute the axis to drag after a pinch event ended", function () {
      ...
      expect(xaxis.min).toBeCloseTo(
        xaxis.c2p(
          xaxis.p2c(previousXmin) +
            (finalCoordsPinch[0].x - finalCoordsPan[0].x),
        ),
        6,
      );
      expect(xaxis.max).toBeCloseTo(
        xaxis.c2p(
          xaxis.p2c(previousXmax) +
            (finalCoordsPinch[0].x - finalCoordsPan[0].x),
        ),
        6,
      );
  ...
  describe("doubleTap", function () {
    ...
    it("should recenter the plot on axis x", function () {
      ...
      simulate.touchstart(
        eventHolder,
        xaxis.p2c(5),
        xaxis.box.top + plot.offset().top + 15,
      );
      simulate.touchend(
        eventHolder,
        xaxis.p2c(5),
