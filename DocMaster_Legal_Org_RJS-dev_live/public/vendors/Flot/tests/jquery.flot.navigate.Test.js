...
describe("flot navigate plugin", function () {
  ...
  describe("zoom", function () {
    ...
    describe("with large numbers", function () {
      it("limits the navigation offsets", function () {
        ...
        expect(
          isFinite(plot.navigationState().yaxis.navigationOffset.below),
        ).toBe(true);
        expect(
          isFinite(plot.navigationState().yaxis.navigationOffset.above),
        ).toBe(true);
      });
    });
  });
  ...
  describe("mouse dblclick", function () {
    ...
    it("sends touched axis for double click on axis", function () {
      ...
      expect(spyRecenter.calls.all()[0].args[0].detail.axisTouched).toEqual(
        xaxis,
      );
    });
  });
