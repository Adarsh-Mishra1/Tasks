...
describe("drawSeries", function () {
  describe("drawSeriesLines", function () {
    ...
    beforeEach(function () {
      ...
      ctx = setFixtures(
        '<div id="test-container" style="width: 200px;height: 100px;border-style: solid;border-width: 1px"><canvas id="theCanvas" style="width: 100%; height: 100%" /></div>',
      )
        .find("#theCanvas")[0]
        .getContext("2d");
      plotWidth = 200;
      ...
      getColorOrGradient = jasmine
        .createSpy()
        .and.returnValue("rgb(10,200,10)");
    });
    ...
    it("should clip the lines when the points are outside the range of the axes", function () {
      ...
      validatePointsAreInsideTheAxisRanges(
        ctx.moveTo.calls.allArgs().concat(ctx.lineTo.calls.allArgs()),
      );
    });
  ...
  describe("drawSeriesPoints", function () {
    ...
    beforeEach(function () {
      ...
      dollar = jasmine
        .createSpy()
        .and.callFake(function (ctx, x, y, radius, shadow) {
          ctx.strokeText("$", x, y);
        });
      getColorOrGradient = jasmine
        .createSpy()
  ...
  describe("drawSeriesBars", function () {
    ...
    it("should draw bars with specified color", function () {
      var fixture = setFixtures(
          '<div id="demo-container" style="width: 800px;height: 600px">',
        )
          .find("#demo-container")
          .get(0),
        placeholder = $(
      ...
      (insideColor1 = getPixelColor(
        ctx,
        ctx.canvas.width / 2,
        ctx.canvas.height / 2,
      )),
        (insideColor2 = getPixelColor(
          ctx,
          ctx.canvas.width / 2 + 35,
          ctx.canvas.height / 2 - 20,
        )),
        (insideColor3 = getPixelColor(
          ctx,
          ctx.canvas.width / 2 - 10,
          ctx.canvas.height / 2 + 30,
        ));

      expect(Array.prototype.slice.call(insideColor1)).toEqual(
        rgba(0, 0, 255, 1),
      );
      expect(Array.prototype.slice.call(insideColor2)).toEqual(
        rgba(0, 0, 255, 1),
      );
      expect(Array.prototype.slice.call(insideColor3)).toEqual(
        rgba(0, 0, 255, 1),
      );
    });
    ...
    it("should use a barWidth based on points distance", function () {
      ...
      for (var i = 0; i < testVector.length; i++) {
        ...
        var barWidth =
          plot.getData()[0].bars.barWidth[0] || plot.getData()[0].bars.barWidth;
    ...
    it("should be able to compute barWidth for NaN, Infinity and -Infinity values", function () {
      var fixture = setFixtures(
          '<div id="test-container" style="width: 600px;height: 400px"><canvas id="theCanvas" style="width: 100%; height: 100%" /></div>',
        ),
