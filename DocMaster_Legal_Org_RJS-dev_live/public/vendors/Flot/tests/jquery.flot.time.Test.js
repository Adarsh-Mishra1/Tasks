...
describe("A Flot chart with absolute time axes", function () {
  ...
  var createPlotWithAbsoluteTimeAxis = function (
    placeholder,
    data,
    formatString,
    base,
  ...
  it("shows time ticks", function () {
    plot = createPlotWithAbsoluteTimeAxis(
      placeholder,
      [
        [
          [0, 1],
  ...
  describe("date generator", function () {
    it("clamps values greater than Date() range to the limit of Date()", function () {
      ...
      expect(dateGenerator(8640000000000001, {}).date).toEqual(
        new Date(8640000000000000),
      );
      expect(dateGenerator(8640000000000002, {}).date).toEqual(
        new Date(8640000000000000),
      );
      expect(dateGenerator(-8640000000000001, {}).date).toEqual(
        new Date(-8640000000000000),
      );
      expect(dateGenerator(-9640000000000000, {}).date).toEqual(
        new Date(-8640000000000000),
      );
    });
  });
  ...
  describe("sub-second ticks", function () {
    it("formats ticks to microsecond accuracy", function () {
      ...
      expect(
        validateTickLabelFormat(plot.getAxes().xaxis.ticks, /00:00:0\d\.\d{6}/),
      ).toEqual(true);
      expect(firstAndLast(plot.getAxes().xaxis.ticks)).toEqual([
    ...
    it("formats ticks to millisecond (3 decimals) accuracy", function () {
      ...
      expect(
        validateTickLabelFormat(plot.getAxes().xaxis.ticks, /00:00:0\d\.\d{3}/),
      ).toEqual(true);
      expect(firstAndLast(plot.getAxes().xaxis.ticks)).toEqual([
    ...
    it("creates ticks automatically to microsecond accuracy", function () {
      ...
      expect(
        validateTickLabelFormat(plot.getAxes().xaxis.ticks, /00\.\d{6}/),
      ).toEqual(true);
      expect(firstAndLast(plot.getAxes().xaxis.ticks)).toEqual([
        { v: 0, label: "00.000000" },
    ...
    it("creates quarter-second ticks", function () {
      ...
      expect(
        validateTickLabelFormat(
          plot.getAxes().xaxis.ticks,
          /0\d\.(00|25|50|75)/,
        ),
      ).toEqual(true);
      expect(firstAndLast(plot.getAxes().xaxis.ticks)).toEqual([
