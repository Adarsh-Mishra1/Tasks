...
describe("composeImages", function () {
  ...
  beforeEach(function () {
    placeholder = setFixtures(
      '<div id="test-container" style="width: 600px;height: 400px; padding: 0px margin: 0px; border: 0px; font-size:0pt; font-family:sans-serif; line-height:0px;">',
    ).find("#test-container");
  ...
  it("should call composeImages on an empty array of sources, so the destination canvas should stay unmodified", function (done) {
    ...
    composeImages(sources, destinationCanvas).then(function () {
      expect(canvasData(destinationCanvas, 80, 10, 1, 1)).toMatchPixelColor([
        0, 0, 0, 255,
      ]);
      expect(destinationCanvas.width).toBe(300);
      expect(destinationCanvas.height).toBe(150);
      expect(canvasData(destinationCanvas, 10, 10, 1, 1)).toMatchPixelColor([
        0, 0, 0, 0,
      ]);
      expect(canvasData(destinationCanvas, 30, 40, 1, 1)).toMatchPixelColor([
        0, 0, 0, 0,
      ]);
      expect(canvasData(destinationCanvas, 50, 70, 1, 1)).toMatchPixelColor([
        0, 0, 0, 0,
      ]);
      expect(canvasData(destinationCanvas, 10, 110, 1, 1)).toMatchPixelColor([
        0, 0, 0, 0,
      ]);
      expect(canvasData(destinationCanvas, 30, 140, 1, 1)).toMatchPixelColor([
        0, 0, 0, 0,
      ]);
      expect(canvasData(destinationCanvas, 50, 170, 1, 1)).toMatchPixelColor([
        0, 0, 0, 0,
      ]);
  ...
  it("should call composeImages on two identical SVGs, one near the other", function (done) {
    ...
    composeImages(sources, destinationCanvas).then(function () {
      ...
      expect(canvasData(destinationCanvas, 110, 10, 1, 1)).toMatchPixelColor([
        255, 0, 0, 255,
      ]); //110 + 4
      expect(canvasData(destinationCanvas, 130, 40, 1, 1)).toMatchPixelColor([
        0, 255, 0, 255,
      ]); //130 + 4
      expect(canvasData(destinationCanvas, 150, 70, 1, 1)).toMatchPixelColor([
        0, 0, 255, 255,
      ]); //150 + 4
  ...
  it("should call composeImages on one canvas as a source", function (done) {
    ...
    composeImages(sources, destinationCanvas).then(function () {
      ...
      expect(canvasData(originalCanvas, 0, 0, 20, 20)).toMatchCanvasArea(
        canvasData(destinationCanvas, 0, 0, 20, 20),
      );
  ...
  it("should call composeImages on one canvas and one SVG", function (done) {
    ...
    composeImages(sources, destinationCanvas).then(function () {
      ...
      expect(canvasData(originalCanvas, 0, 0, 20, 20)).toMatchCanvasArea(
        canvasData(destinationCanvas, 0, 80, 20, 20),
      );

      expect(
        canvasData(destinationCanvas, 20 + 10, 10, 1, 1),
      ).toMatchPixelColor([255, 0, 0, 255]); //24 + 10
      expect(
        canvasData(destinationCanvas, 20 + 30, 40, 1, 1),
      ).toMatchPixelColor([0, 255, 0, 255]); //24 + 30
      expect(
        canvasData(destinationCanvas, 20 + 50, 70, 1, 1),
      ).toMatchPixelColor([0, 0, 255, 255]); //24 + 50
  ...
  it("should call composeImages on two canvases", function (done) {
    ...
    composeImages(sources, destinationCanvas).then(function () {
      ...
      expect(canvasData(originalCanvas1, 0, 0, 20, 20)).toMatchCanvasArea(
        canvasData(destinationCanvas, 0, 0, 20, 20),
      );

      expect(canvasData(originalCanvas2, 0, 0, 20, 20)).toMatchCanvasArea(
        canvasData(destinationCanvas, 20 + 0, 0, 20, 20),
      ); //20 + 4
  ...
  it("should call composeImages on two partially overlapped canvases", function (done) {
    ...
    composeImages(sources, destinationCanvas).then(function () {
      ...
      expect(canvasData(originalCanvas2, 0, 0, 20, 20)).toMatchCanvasArea(
        canvasData(destinationCanvas, 20 + 0 - 10, 0, 20, 20),
      ); //20 + 10 - 4
  ...
  it("should call composeImages on two partially overlapped canvases. Same as above test, but the two canvases have the opposite Z order.", function (done) {
    ...
    composeImages(sources, destinationCanvas).then(function () {
      ...
      expect(canvasData(originalCanvas2, 0, 0, 20 - 10 + 0, 20)) //20 - 10 + 4
        .toMatchCanvasArea(
          canvasData(destinationCanvas, 20, 0, 20 - 10 + 0, 20),
        ); //20 - 10 + 4
  ...
  it("should call composeImages on two separate canvases, where one canvas is outside of view area", function (done) {
    ...
    composeImages(sources, destinationCanvas).then(function () {
      ...
      expect(canvasData(originalCanvas1, 0, 0, 20, 20)).toMatchCanvasArea(
        canvasData(destinationCanvas, 100 - 20 - 0, 0, 20, 20),
      ); //100 - 20 - 10 + 6
  ...
  it("should call composeImages on one canvas and an SVG, which are totally overlapped with transparency", function (done) {
    ...
    composeImages(sources, destinationCanvas).then(function () {
      ...
      expect(canvasData(originalCanvas1, 0, 0, 20, 20)).toMatchCanvasArea(
        canvasData(destinationCanvas, 100 - 40 + 0, 0, 20, 20),
      ); //100 - 40 + 4
  ...
  it("should call composeImages on one canvas and an SVG, which are totally overlapped with transparency. The SVG has a different size than the ones from other tests. One component of the SVG is partially outside of the view area.", function (done) {
    ...
    composeImages(sources, destinationCanvas).then(function () {
      ...
      expect(canvasData(originalCanvas1, 0, 0, 20, 20)).toMatchCanvasArea(
        canvasData(destinationCanvas, 250 - 180 + 0, 150 - 10 - 20, 20, 20),
      ); //250 - 180 + 4

      //circle centers
      expect(canvasData(destinationCanvas, 230, 20, 1, 1)).toMatchPixelColor([
        255, 0, 0, 255,
      ]);
      expect(canvasData(destinationCanvas, 175, 100, 1, 1)).toMatchPixelColor([
        0, 255, 0, 255,
      ]);
      expect(canvasData(destinationCanvas, 50, 130, 1, 1)).toMatchPixelColor([
        0, 0, 255, 255,
      ]);

      //other points on circles should match the required colors, because of configured diameters
      expect(canvasData(destinationCanvas, 220, 17, 1, 1)).toMatchPixelColor([
        255, 0, 0, 255,
      ]);
      expect(canvasData(destinationCanvas, 190, 114, 1, 1)).toMatchPixelColor([
        0, 255, 0, 255,
      ]);
      expect(canvasData(destinationCanvas, 80, 149, 1, 1)).toMatchPixelColor([
        0, 0, 255, 255,
      ]);
      ...
      expect(
        canvasData(destinationCanvas, 79, 102, 1, 1),
      ).toMatchPixelColorWithError([0, 0, 0, 255, 15]);
