describe("flot hover plugin", function () {
  ...
  describe("mouse hover", function () {
    ...
    it("should highlight the point when hovered", function () {
      ...
      expect(getEntireCanvasData(canvas)).toContainPixelColor(
        rgba(10, 20, 30, 1),
      );
    });
    ...
    it("should not highlight the point when hovered and the grid is not hoverable", function () {
      ...
      expect(getEntireCanvasData(canvas)).not.toContainPixelColor(
        rgba(10, 20, 30, 1),
      );
    });
    ...
    it("should unhighlight the previouse point when hovering a new one", function () {
      ...
      expect(
        canvasData(canvas, x1 - r, y1 - r, 2 * r, 2 * r),
      ).toContainPixelColor(rgba(10, 20, 30, 1));
      expect(
        canvasData(canvas, x2 - r, y2 - r, 2 * r, 2 * r),
      ).not.toContainPixelColor(rgba(10, 20, 30, 1));
    ...
    it("should update the current hover point to the placeholder when the plot created again", function () {
      ...
      expect(
        plot.getPlaceholder()[0].lastMouseMoveEvent.originalEvent.x,
      ).toEqual(evt.x);
      expect(
        plot.getPlaceholder()[0].lastMouseMoveEvent.originalEvent.y,
      ).toEqual(evt.y);
    });
