...
(function () {
  ...
  $P.processTimeObject = function (obj) {
    ...
    if (obj.zone) {
      ...
      if (
        ...
        offset =
          obj.zone_hours * 60 + (obj.zone_minutes ? obj.zone_minutes : 0);
        if (obj.zone_sign === "+") {
          offset *= -1;
        }
        offset -= date.getTimezoneOffset();
  ...
  $P.ISO = {
    ...
    parse: function (s) {
      ...
      var time = {
        ...
        weekDay: data[9]
          ? Math.abs(Number(data[9])) === 7
            ? 0
            : Math.abs(Number(data[9]))
          : data[9], // 1-7, starts on Monday. Convert to JS's 0-6 index.
        ...
        seconds: data[19]
          ? Math.floor(Number(data[19].replace(":", "").replace(",", ".")))
          : data[19],
        milliseconds: data[20]
          ? Number(data[20].replace(",", ".")) * 1000
          : data[20],
        zone: data[21],
        zone_sign: data[22],
        zone_hours:
          data[23] && typeof data[23] !== "undefined"
            ? Number(data[23])
            : data[23],
        zone_minutes:
          data[24] && typeof data[23] !== "undefined"
            ? Number(data[24])
            : data[24],
      };
      if (data[18]) {
  ...
  $P.Normalizer = {
    parse: function (s) {
      ...
      s = s.replace(
        new RegExp("((" + $R.past.source + ")\\s(" + $R.mon.source + "))"),
        Date.today().last().monday().toString("d"),
      );
      s = s.replace(
        new RegExp("((" + $R.past.source + ")\\s(" + $R.tue.source + "))"),
        Date.today().last().tuesday().toString("d"),
      );
      s = s.replace(
        new RegExp("((" + $R.past.source + ")\\s(" + $R.wed.source + "))"),
        Date.today().last().wednesday().toString("d"),
      );
      s = s.replace(
        new RegExp("((" + $R.past.source + ")\\s(" + $R.thu.source + "))"),
        Date.today().last().thursday().toString("d"),
      );
      s = s.replace(
        new RegExp("((" + $R.past.source + ")\\s(" + $R.fri.source + "))"),
        Date.today().last().friday().toString("d"),
      );
      s = s.replace(
        new RegExp("((" + $R.past.source + ")\\s(" + $R.sat.source + "))"),
        Date.today().last().saturday().toString("d"),
      );
      s = s.replace(
        new RegExp("((" + $R.past.source + ")\\s(" + $R.sun.source + "))"),
        Date.today().last().sunday().toString("d"),
      );
...
(function () {
  ...
  g["ss.s"] = _.cache(
    _.process(_.rtoken(/^[0-5][0-9]\.[0-9]{1,3}/), t.secondAndMillisecond),
  );
...
var TimePeriod = function (
  ...
  var attrs = "years months days hours minutes seconds milliseconds".split(
    /\s+/,
  );
