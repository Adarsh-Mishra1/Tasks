...
var buildMinifyFileList = function (dev) {
  ...
  files.map(function (item) {
    ...
    var dest =
      dirs.build + "/" + output_path + file_core_name + output_ext + "js";
    output[dest] = [dirs.build + "/" + file_core_name + ".js"];
    return dest;
  });
  output[dirs.build + "/" + output_path + "date" + output_ext + "js"] = [
    dirs.build + "/" + "date.js",
  ];
  return output;
};
...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    build_dev: {
      description:
        "Builds files designed for easy debugging on dev enviroments (non-minified)",
    },
    build_prod: {
  ...
  grunt.registerMultiTask(
    "i18n",
    "Wraps DateJS core with Internationalization info.",
    function () {
      var data = this.data,
        path = require("path"),
  ...
  grunt.registerMultiTask(
    "build_dev",
    "Builds compiled, non-minfied, files for development enviroments",
    function () {
  ...
  grunt.registerMultiTask(
    "build_prod",
    "Rebuilds dev and minifies files for production enviroments",
    function () {
      grunt.task.run([
        "concat:core",
        "concat:basic",
