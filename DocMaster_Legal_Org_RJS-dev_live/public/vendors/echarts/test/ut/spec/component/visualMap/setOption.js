describe("vsiaulMap_setOption", function () {
    ...
    testCase.createChart()("defaultTargetController", function () {
        ...
        expect(option.visualMap[0].target.inRange.color).toEqual([
            "red",
            "blue",
            "yellow",
        ]);
        expect(option.visualMap[0].controller.inRange.color).toEqual([
            "red",
            "blue",
            "yellow",
        ]);
    ...
    testCase.createChart()("eraseAllVisualProps_notRelative", function () {
        ...
        expect(option.visualMap.length).toEqual(1);
        expect(option.visualMap[0].inRange.hasOwnProperty("color")).toEqual(
            false,
        );
        expect(
            option.visualMap[0].target.inRange.hasOwnProperty("color"),
        ).toEqual(false);
        expect(
            option.visualMap[0].controller.inRange.hasOwnProperty("color"),
        ).toEqual(false);
        expect(option.visualMap[0].inRange.symbolSize).toEqual([0.4, 0.6]);
        expect(option.visualMap[0].target.inRange.symbolSize).toEqual([
            0.4, 0.6,
        ]);
    ...
    testCase.createChart()("eraseAllVisualProps_reletive", function () {
        ...
        expect(option.visualMap[0].target.inRange.colorAlpha).toEqual([
            0.4, 0.6,
        ]);
        expect(option.visualMap[0].controller.inRange.colorAlpha).toEqual([
            0.4, 0.6,
        ]);
        ...
        expect(option.visualMap.length).toEqual(1);
        expect(
            option.visualMap[0].target.inRange.hasOwnProperty("colorAlpha"),
        ).toEqual(false);
        expect(
            option.visualMap[0].controller.inRange.hasOwnProperty("colorAlpha"),
        ).toEqual(false);
        expect(option.visualMap[0].target.inRange.color).toEqual([
            "green",
            "blue",
        ...
        expect(option.visualMap[0].controller.outOfRange.symbol).toEqual([
            "diamond",
        ]);
    });
    ...
    testCase.createChart(2)("normalizeVisualRange", function () {
        ...
        function getVisual(idx, visualType) {
            return ecModel.getComponent("visualMap", idx).targetVisuals.inRange[
                visualType
            ].option.visual;
        }
