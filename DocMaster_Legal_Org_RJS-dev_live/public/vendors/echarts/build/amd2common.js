...
function getRequires(args, factory) {
    ...
    params.forEach(function (param) {
        if (MAGIC_DEPS[param.dep] && !MAGIC_DEPS[param.name]) {
            ...
            requires.push(
                "var " + param.name + " = require('" + param.dep + "');",
            );
        }
    });
...
function getBody(raw, factoryBody, useStrict) {
    ...
    var body = "";
    var bodyStart = useStrict
        ? useStrict.expression.range[1] + 1
        : factoryBody.range[0] + 1;
    ...
    if (returnStatement) {
        ...
        body +=
            "module.exports =" +
            raw.substring(
                returnStatement.range[0] + 6,
                factoryBody.range[1] - 1,
            );
    } else {
