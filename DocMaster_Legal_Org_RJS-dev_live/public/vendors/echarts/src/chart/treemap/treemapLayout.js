define(function (require) {
        ...
        ecModel.eachComponent(condition, function (seriesModel) {
            ...
            if (payloadType !== "treemapMove") {
                ...
                each(viewAbovePath, function (node, index) {
                    var childValue = (
                        viewAbovePath[index + 1] || viewRoot
                    ).getValue();
                    node.setLayout(
                        zrUtil.extend(
                            {
    ...
    function initChildren(
        ...
        var overLeafDepth =
            options.leafDepth != null && options.leafDepth <= depth;
    ...
    function filterByThreshold(
        ...
        for (var i = len - 1; i >= 0; i--) {
            var value =
                orderedChildren[orderBy === "asc" ? len - i - 1 : i].getValue();
    ...
    function position(row, rowFixedLength, rect, halfGapWidth, flush) {
        ...
        for (var i = 0, rowLen = row.length; i < rowLen; i++) {
            ...
            var step = rowOtherLength
                ? node.getLayout().area / rowOtherLength
                : 0;

            var wh1 = (nodeLayout[wh[idx1WhenH]] = mathMax(
            ...
            var wh0 = (nodeLayout[wh[idx0WhenH]] = mathMax(
                modWH - 2 * halfGapWidth,
                0,
            ));

            nodeLayout[xy[idx1WhenH]] =
                rect[xy[idx1WhenH]] + mathMin(halfGapWidth, wh1 / 2);
        ...
        while ((parent = currNode.parentNode)) {
            ...
            if (isFinite(borderWidth)) {
                ...
                area +=
                    ...
                    4 * borderWidth * Math.pow(area, 0.5);
            }

            area > numberUtil.MAX_SAFE_INTEGER &&
                (area = numberUtil.MAX_SAFE_INTEGER);
    ...
    function prunning(node, clipRect, viewAbovePath, viewRoot, depth) {
        ...
        var isAboveViewRoot =
            nodeInViewAbovePath && nodeInViewAbovePath === node;
