        ...
        zrUtil.each(mapSeries, function (seriesOpt) {
            ...
            if (seriesOpt.markPoint) {
                ...
                if (!(seriesOpt.data && seriesOpt.data.length)) {
                    ...
                    if (!geoOpt) {
                        geoOpt = newCreatedGeoOptMap[seriesOpt.map] =
                            createGeoFromMap(seriesOpt);
                        option.geo.push(geoOpt);
                    }

                    var scatterSeries = seriesOpt.markPoint;
                    scatterSeries.type =
                        option.effect && option.effect.show
                            ? "effectScatter"
                            : "scatter";
                    ...
                    option.series.splice(
                        zrUtil.indexOf(option.series, seriesOpt),
                        1,
                        scatterSeries,
                    );
                }
