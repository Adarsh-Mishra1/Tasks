    ...
    function updateMapSelectHandler(mapOrGeoModel, group, api, fromView) {
        ...
        mapOrGeoModel.get("selectedMode") &&
            group.on("click", function (e) {
                ...
                var action = {
                    type:
                        (mapOrGeoModel.mainType === "geo" ? "geo" : "map") +
                        "ToggleSelect",
                    name: region.name,
                    from: fromView.uid,
                };
    ...
    function updateMapSelected(mapOrGeoModel, group) {
        ...
        group.eachChild(function (otherRegionEl) {
            if (otherRegionEl.__region) {
                otherRegionEl.trigger(
                    mapOrGeoModel.isSelected(otherRegionEl.__region.name)
                        ? "emphasis"
                        : "normal",
                );
            }
    ...
    MapDraw.prototype = {
        ...
        draw: function (mapOrGeoModel, ecModel, api, fromView, payload) {
            ...
            zrUtil.each(geo.regions, function (region) {
                ...
                var regionModel =
                    mapOrGeoModel.getRegionModel(region.name) || mapOrGeoModel;

                var itemStyleModel = regionModel.getModel(itemStyleAccessPath);
                var hoverItemStyleModel = regionModel.getModel(
                    hoverItemStyleAccessPath,
                );
                var itemStyle = getFixedItemStyle(itemStyleModel, scale);
                var hoverItemStyle = getFixedItemStyle(
                    hoverItemStyleModel,
                    scale,
                );
                ...
                var hoverLabelModel =
                    regionModel.getModel(hoverLabelAccessPath);
                ...
                if (data) {
                    ...
                    var visualColor = data.getItemVisual(
                        dataIdx,
                        "color",
                        true,
                    );
                ...
                if (
                    !data ||
                    (isDataNaN && (showLabel || hoverShowLabel)) ||
                    (itemLayout && itemLayout.showLabel)
                ) {
                    var query = data ? dataIdx : region.name;
                    var formattedStr = mapOrGeoModel.getFormattedLabel(
                        query,
                        "normal",
                    );
                    var hoverFormattedStr = mapOrGeoModel.getFormattedLabel(
                        query,
                        "emphasis",
                    );
