define(function (require) {
    ...
    var handlers = {
        zoom: function (controllerGroup, featureModel, ecModel, api) {
            ...
            interactionMutex[isZoomActive ? "take" : "release"](
                "globalPan",
                zr,
            );
    ...
    function prepareCoordInfo(grid, ecModel) {
        ...
        ecModel.eachComponent(
            ...
            function (dzModel, dataZoomIndex) {
                if (
                    isTheAxis("xAxis", coordInfo[0].axisModel, dzModel, ecModel)
                ) {
                    coordInfo[0].dataZoomModel = dzModel;
                }
                if (
                    isTheAxis("yAxis", coordInfo[1].axisModel, dzModel, ecModel)
                ) {
        ...
        ecModel.eachComponent(
            ...
            function (gridModel, gridIndex) {
                ...
                if (selDataRange) {
                    ...
                    xBatchItem &&
                        (snapshot[xBatchItem.dataZoomId] = xBatchItem);
                    yBatchItem &&
                        (snapshot[yBatchItem.dataZoomId] = yBatchItem);
                }
            },
            this,
        );
    ...
    function pointToDataInCartesian(selRange, coordInfo) {
        ...
        var cartesian = grid.getCartesian(
            coordInfo[0].axisIndex,
            coordInfo[1].axisIndex,
        );
        var dataLeftTop = cartesian.pointToData(
            [selRange[0][0], selRange[1][0]],
            true,
        );
        var dataRightBottom = cartesian.pointToData(
            [selRange[0][1], selRange[1][1]],
            true,
        );
    ...
    require("../../../echarts").registerPreprocessor(function (option) {
        ...
        function addForAxis(axisName, dataZoomOpt) {
            ...
            if (givenAxisIndices != null && !zrUtil.isArray(givenAxisIndices)) {
                givenAxisIndices =
                    givenAxisIndices === false ? [] : [givenAxisIndices];
            }

            forEachComponent(axisName, function (axisOpt, axisIndex) {
                if (
                    givenAxisIndices != null &&
