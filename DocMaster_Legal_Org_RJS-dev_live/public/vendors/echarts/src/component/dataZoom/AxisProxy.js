...
define(function (require) {
    ...
    AxisProxy.prototype = {
        ...
        getTargetSeriesModels: function () {
            ...
            this.ecModel.eachSeries(function (seriesModel) {
                if (
                    this._axisIndex ===
                    seriesModel.get(this._dimName + "AxisIndex")
                ) {
        ...
        getOtherAxisModel: function () {
            ...
            ecModel.eachComponent(
                otherAxisDim + "Axis",
                function (otherAxisModel) {
                    if (
                        (otherAxisModel.get(coordSysIndexName) || 0) ===
        ...
        filterData: function (dataZoomModel) {
            ...
            each(seriesModels, function (seriesModel) {
                ...
                seriesData &&
                    each(
                        seriesModel.coordDimToDataDim(axisDim),
                        function (dim) {
                            if (filterMode === "empty") {
                                seriesModel.setData(
    ...
    function fixExtendByAxis(dataExtent, axisModel, scale) {
        each(["min", "max"], function (minMax, index) {
            ...
            if (
                axisMax != null &&
                (axisMax + "").toLowerCase() !== "data" + minMax
            ) {
    ...
    function setAxisModel(axisProxy, isRestore) {
        ...
        var isFull =
            isRestore || (percentWindow[0] === 0 && percentWindow[1] === 100);
        // [0, 500]: arbitrary value, guess axis extent.
        var precision =
            !isRestore && numberUtil.getPixelPrecision(valueWindow, [0, 500]);
        // toFixed() digits argument must be between 0 and 20
        var invalidPrecision =
            !isRestore && !(precision < 20 && precision >= 0);
