define(function (require) {
    ...
    var AxisView = require("../../echarts").extendComponentView({
        ...
        _splitArea: function (axisModel, gridModel, labelInterval) {
            ...
            for (var i = 1; i < ticksCoords.length; i++) {
                ...
                splitAreaRects[colorIndex].push(
                    new graphic.Rect({
                        shape: {
                            x: x,
                            y: y,
            ...
            for (var i = 0; i < splitAreaRects.length; i++) {
                this.group.add(
                    graphic.mergePath(splitAreaRects[i], {
                        style: zrUtil.defaults(
                            {
    ...
    function layoutAxis(gridModel, axisModel) {
        ...
        posMap.x.onZero = Math.max(
            Math.min(getZero("y"), posMap.x.bottom),
            posMap.x.top,
        );
        posMap.y.onZero = Math.max(
            Math.min(getZero("x"), posMap.y.right),
            posMap.y.left,
        );
        ...
        if (axis.onZero) {
            layout.labelOffset =
                posMap[axisDim][rawAxisPosition] - posMap[axisDim].onZero;
        }
