define(function (require) {
    ...
    function getPrecision(data, valueAxisDim, dataIndex) {
        ...
        do {
            precision = Math.max(
                numberUtil.getPrecision(data.get(valueAxisDim, dataIndex)),
                precision,
            );
            data = data.stackedOn;
        } while (data);
    ...
    function markerTypeCalculatorWithExtent(
        ...
        if (precision >= 0) {
            coordArr[valueCoordIndex] =
                +coordArr[valueCoordIndex].toFixed(precision);
        }
    ...
    var dataTransform = function (seriesModel, item) {
        ...
        if (
            item &&
            (isNaN(parseFloat(item.x)) || isNaN(parseFloat(item.y))) &&
    ...
    var getAxisInfo = function (item, data, coordSys, seriesModel) {
        ...
        if (item.valueIndex != null || item.valueDim != null) {
            ...
            ret.valueAxis = coordSys.getAxis(
                seriesModel.dataDimToCoordDim(ret.valueDataDim),
            );
            ret.baseAxis = coordSys.getOtherAxis(ret.valueAxis);
            ret.baseDataDim = seriesModel.coordDimToDataDim(
                ret.baseAxis.dim,
            )[0];
        } else {
            ...
            ret.valueDataDim = seriesModel.coordDimToDataDim(
                ret.valueAxis.dim,
            )[0];
        }
