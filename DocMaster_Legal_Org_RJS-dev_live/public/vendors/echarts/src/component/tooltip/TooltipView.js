    ...
    require("../../echarts").extendComponentView({
            ...
            if (event.x == null || event.y == null) {
                if (!seriesModel) {
                    ...
                    ecModel.eachSeries(function (_series) {
                        if (
                            ifSeriesSupportAxisTrigger(_series) &&
                            !seriesModel
                        ) {
                            seriesModel = _series;
                        }
                        ...
                        var point = coordSys.dataToPoint(
                            data.getValues(
                                zrUtil.map(coordSys.dimensions, function (dim) {
                                    return seriesModel.coordDimToDataDim(
                                        dim,
                                    )[0];
                                }),
                                dataIndex,
        ...
        _tryShow: function (e) {
            ...
            if (el && el.dataIndex != null) {
                ...
                var dataModel =
                    el.dataModel || ecModel.getSeriesByIndex(el.seriesIndex);
                var dataIndex = el.dataIndex;
                var itemModel = dataModel.getData().getItemModel(dataIndex);
                // Series or single data may use item trigger when global is axis trigger
                if (
                    (itemModel.get("tooltip.trigger") || globalTrigger) ===
                    "axis"
                ) {
        ...
        _showAxisTooltip: function (tooltipModel, ecModel, e) {
            ...
            zrUtil.each(
                ...
                function (seriesCoordSysSameAxis) {
         ...
         * @private
         */
        _showCartesianPointer: function (
            axisPointerModel,
            cartesian,
            axisType,
            point,
            ...
            if (axisPointerType === "cross") {
                moveGridLine(
                    "x",
                    point,
                    cartesian.getAxis("y").getGlobalExtent(),
                );
                moveGridLine(
                    "y",
                    point,
                    cartesian.getAxis("x").getGlobalExtent(),
                );
                ...
                if (cartesian.type === "cartesian2d") {
                    (axisPointerType === "line"
                        ? moveGridLine
                        : moveGridShadow)(axisType, point, otherExtent);
                }
            }
        ...
        _showPolarPointer: function (axisPointerModel, polar, axisType, point) {
            ...
            if (axisPointerType === "cross") {
                ...
                var otherAxis = polar.getAxis(
                    axisType === "radius" ? "angle" : "radius",
                );
        ...
        _getPointerElement: function (
            ...
            var style =
                styleModel[isShadow ? "getAreaStyle" : "getLineStyle"]();
            ...
            var el = (axisPointers[coordSysName][axisType] = new graphic[
                elementType
            ]({
                style: style,
                z: z,
        ...
        _dispatchAndShowSeriesTooltipContent: function (
                ...
                return {
                    ...
                    dataIndex: series.getAxisTooltipDataIndex
                        ? series.getAxisTooltipDataIndex(
                              series.coordDimToDataDim(baseAxis.dim),
                              value,
                              baseAxis,
                          )
                        : series
            ...
            if (
                baseAxis &&
                rootTooltipModel.get("showContent") &&
                rootTooltipModel.get("show")
            ) {
                var formatter = rootTooltipModel.get("formatter");
                var positionExpr = rootTooltipModel.get("position");
                var html;

                var paramsList = zrUtil.map(
                    seriesList,
                    function (series, index) {
                        return series.getDataParams(
                            payloadBatch[index].dataIndex,
                        );
                    },
                );
                // If only one series
                ...
                if (!contentNotChange) {
                    ...
                    if (!formatter) {
                        ...
                        var firstLine = seriesList[0]
                            .getData()
                            .getName(firstDataIndex);
                        html =
                            (firstLine ? firstLine + "<br />" : "") +
                            zrUtil
                                .map(seriesList, function (series, index) {
                                    return series.formatTooltip(
                                        payloadBatch[index].dataIndex,
                                        true,
                                    );
                                })
                        ...
                        if (typeof formatter === "string") {
                            ...
                            var self = this;
                            var ticket =
                                "axis_" + coordSys.name + "_" + firstDataIndex;
         ...
         * @param {Object} e
         */
        _showItemTooltipContent: function (
            seriesModel,
            dataIndex,
            dataType,
            e,
            ...
            if (tooltipModel.get("showContent") && tooltipModel.get("show")) {
                ...
                var html;
                if (!formatter) {
                    html = seriesModel.formatTooltip(
                        dataIndex,
                        false,
                        dataType,
                    );
                    ...
                    if (typeof formatter === "string") {
                        ...
                        var self = this;
                        var ticket =
                            "item_" + seriesModel.name + "_" + dataIndex;
                        var callback = function (cbTicket, html) {
                            if (cbTicket === self._ticket) {
                                tooltipContent.setContent(html);
        ...
        _showAxisPointer: function (coordSysName) {
            if (coordSysName) {
                ...
                axisPointers &&
                    zrUtil.each(axisPointers, function (el) {
                        el.show();
                    });
            } else {
