...
define(function (require) {
    ...
    function Radar(radarModel, ecModel, api) {
        ...
        this._indicatorAxes = zrUtil.map(
    ...
    Radar.prototype.dataToPoint = function (value, indicatorIndex) {
        ...
        return this.coordToPoint(
            indicatorAxis.dataToCoord(value),
            indicatorIndex,
        );
    };
    ...
    Radar.prototype.resize = function (radarModel, api) {
        ...
        zrUtil.each(
            ...
            function (indicatorAxis, idx) {
                ...
                var angle =
                    this.startAngle +
                    (idx * Math.PI * 2) / this._indicatorAxes.length;
                // Normalize to [-PI, PI]
    ...
    Radar.prototype.update = function (ecModel, api) {
        ...
        ecModel.eachSeriesByType(
            ...
            function (radarSeries, idx) {
                if (
                    ...
                    ecModel.getComponent(
                        "radar",
                        radarSeries.get("radarIndex"),
                    ) !== radarModel
                ) {
                    return;
                }
                ...
                zrUtil.each(indicatorAxes, function (indicatorAxis) {
                    indicatorAxis.scale.unionExtent(
                        data.getDataExtent(indicatorAxis.dim),
                    );
                });
            },
            this,
        ...
        function increaseInterval(interval) {
            var exp10 = Math.pow(
                10,
                Math.floor(Math.log(interval) / Math.LN10),
            );
            // Increase interval
        ...
        zrUtil.each(indicatorAxes, function (indicatorAxis, idx) {
            var rawExtent = axisHelper.getScaleExtent(
                indicatorAxis,
                indicatorAxis.model,
            );
            ...
            if (fixedMin != null && fixedMax != null) {
                ...
                // PENDING
                var center =
                    Math.round((rawExtent[0] + rawExtent[1]) / 2 / interval) *
                    interval;
                var halfSplitNumber = Math.round(splitNumber / 2);
                scale.setExtent(
                    numberUtil.round(center - halfSplitNumber * interval),
                    numberUtil.round(
                        center + (splitNumber - halfSplitNumber) * interval,
                    ),
                );
                scale.setInterval(interval);
            }
