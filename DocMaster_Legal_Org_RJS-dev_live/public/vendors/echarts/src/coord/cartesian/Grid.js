...
define(function (require, factory) {
    ...
    function isAxisUsedInTheGrid(axisModel, gridModel, ecModel) {
        return (
            ecModel.getComponent("grid", axisModel.get("gridIndex")) ===
            gridModel
        );
    }
    ...
    gridProto.update = function (ecModel, api) {
        ...
        function ifAxisCanNotOnZero(otherAxisDim) {
            ...
            for (var idx in axes) {
                ...
                if (
                    axis &&
                    (axis.type === "category" || !ifAxisCrossZero(axis))
                ) {
                    return true;
                }
            }
    ...
    gridProto._initCartesian = function (gridModel, ecModel, api) {
        ...
        each(
            axesMap.x,
            function (xAxis, xAxisIndex) {
                each(
                    axesMap.y,
                    function (yAxis, yAxisIndex) {
                        var key = "x" + xAxisIndex + "y" + yAxisIndex;
    ...
    gridProto._updateScale = function (ecModel, gridModel) {
        ...
        ecModel.eachSeries(function (seriesModel) {
            if (seriesModel.get("coordinateSystem") === "cartesian2d") {
                ...
                if (
                    !isAxisUsedInTheGrid(xAxisModel, gridModel, ecModel) ||
        ...
        function unionExtent(data, axis, seriesModel) {
            each(seriesModel.coordDimToDataDim(axis.dim), function (dim) {
                axis.scale.unionExtent(
                    data.getDataExtent(dim, axis.scale.type !== "ordinal"),
                );
            });
        }
    };
