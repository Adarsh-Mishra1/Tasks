  ...
  UWidget.prototype = {
    ...
    init: function ($element, options) {
      ...
      if (!this.options.url || !this.options.handler || !this.options.template)
        throw new Error(
          "You must define a widget url, an ajax handler and a template",
        );

      this._initActions()._initFromDOM().fetch();
    },
    ...
    _initActions: function () {
      ...
      if (this.options.sort.enabled) {
        this.$sort = $(
          '<select name="' + this.options.sort.name + '"></select>',
        ).on("change", false, $.proxy(this._updateActions, this));

        for (i = 0; i < this.options.sort.values.length; i++)
          this.$sort.append(
            '<option value="' +
              this.options.sort.values[i] +
              '">' +
              this.options.sort.labels[i] +
              "</option>",
          );
      ...
      if (this.options.direction.enabled) {
        this.$direction = $(
          '<select name="' + this.options.direction.name + '"></select>',
        ).on("change", false, $.proxy(this._updateActions, this));

        for (i = 0; i < this.options.direction.values.length; i++)
          this.$direction.append(
            '<option value="' +
              this.options.direction.values[i] +
              '">' +
              this.options.direction.labels[i] +
              "</option>",
          );
      ...
      if (this.options.filters.enabled) {
        this.$filters = $('<span class="filters"></span>').on(
          "change",
          false,
          $.proxy(this._updateActions, this),
        );

        for (i = 0; i < this.options.filters.values.length; i++) {
          checked =
            this.$element.data("filters") &&
            new RegExp(this.options.filters.labels[i], "i").test(
              this.$element.data("filters"),
            );
          this.$filters.append(
            this.options.filters.labels[i] +
              ' <input type="checkbox" name="filters[]" value="' +
              this.options.filters.values[i] +
              '" ' +
              (checked ? "checked" : "") +
              "/>",
          );
      ...
      this.$info = $(
        '<span class="uwidget-info"><a href="#" target="_blank">UWidget</a></span>',
      );
    ...
    _initFromDOM: function () {
      ...
      if (this.$element.data("height")) {
        ...
        this.$container.css(
          "height",
          this.$element.height() - this.$actions.height() - this.$info.height(),
        );
      }
