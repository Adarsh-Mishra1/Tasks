			...
			function (require, module, exports) {
				...
				(function (global, factory) {
					...
					var momentProperties =
						(utils_hooks__hooks.momentProperties = []);
					...
					function warn(msg) {
						if (
							utils_hooks__hooks.suppressDeprecationWarnings ===
								false &&
					...
					function deprecate(msg, fn) {
						...
						return extend(function () {
							if (utils_hooks__hooks.deprecationHandler != null) {
								utils_hooks__hooks.deprecationHandler(
									null,
									msg,
								);
							}
					...
					function configFromArray(config) {
						...
						if (config._dayOfYear) {
							...
							if (config._dayOfYear > daysInYear(yearToUse)) {
								getParsingFlags(config)._overflowDayOfYear =
									true;
							}
					...
					var prototypeMin = deprecate(
						...
						function () {
							var other = local__createLocal.apply(
								null,
								arguments,
							);
						...
						if (model._isUTC) {
							...
							diff =
								(isMoment(input) || isDate(input)
									? input.valueOf()
									: local__createLocal(input).valueOf()) -
								res.valueOf();
					...
					function add_subtract__addSubtract(
						...
						if (months) {
							setMonth(
								mom,
								get_set__get(mom, "Month") + months * isAdding,
							);
						}
						if (updateOffset) {
							utils_hooks__hooks.updateOffset(
								mom,
								days || months,
							);
						}
					...
					momentPrototype__proto.toISOString =
					...
					// Quarter
					momentPrototype__proto.quarter =
						momentPrototype__proto.quarters = getSetQuarter;
					...
					momentPrototype__proto.week = momentPrototype__proto.weeks =
						getSetWeek;
					momentPrototype__proto.isoWeek =
						momentPrototype__proto.isoWeeks = getSetISOWeek;
					...
					// Minute
					momentPrototype__proto.minute =
						momentPrototype__proto.minutes = getSetMinute;

					// Second
					momentPrototype__proto.second =
						momentPrototype__proto.seconds = getSetSecond;
					...
					prototype__proto._monthsShortRegex =
					...
					prototype__proto._weekdaysShort =
						defaultLocaleWeekdaysShort;
					prototype__proto.weekdaysParse = localeWeekdaysParse;

					prototype__proto._weekdaysRegex = defaultWeekdaysRegex;
					prototype__proto.weekdaysRegex = weekdaysRegex;
					prototype__proto._weekdaysShortRegex =
						defaultWeekdaysShortRegex;
					prototype__proto.weekdaysShortRegex = weekdaysShortRegex;
					prototype__proto._weekdaysMinRegex =
						defaultWeekdaysMinRegex;
					prototype__proto.weekdaysMinRegex = weekdaysMinRegex;

					// Hours
					prototype__proto.isPM = localeIsPM;
					prototype__proto._meridiemParse =
					...
					function lists__listWeekdaysShort(
						localeSorted,
						format,
						index,
					) {
					...
					function duration_humanize__relativeTime(
						...
						var duration =
							create__createDuration(posNegDuration).abs();
					...
					function iso_string__toISOString() {
						...
						var seconds =
							iso_string__abs(this._milliseconds) / 1000;
					...
					duration_prototype__proto.toString =
						iso_string__toISOString;
								...
								label: function (tooltipItem, data) {
									var datasetLabel =
										data.datasets[tooltipItem.datasetIndex]
											.label || "";
									return (
										datasetLabel + ": " + tooltipItem.xLabel
									);
								},
							},
						},
						...
						Chart.controllers.bar.extend({
							...
							getRuler: function (index) {
								...
								if (yScale.options.type === "category") {
									...
									// Average width
									tickHeight =
										yScale.width / yScale.ticks.length;
								}
								var categoryHeight =
									tickHeight *
						...
						setHoverStyle: function (point) {
							...
							model.radius = custom.hoverRadius
								...
								: helpers.getValueAtIndexOrDefault(
									...
									this.getRadius(
										this.getDataset().data[point._index],
									);
							model.backgroundColor = custom.hoverBackgroundColor
								? custom.hoverBackgroundColor
								: helpers.getValueAtIndexOrDefault(
						...
						Chart.DatasetController.extend({
							...
							update: function update(reset) {
								...
								_this.outerRadius =
									chart.outerRadius -
									chart.radiusLength *
										_this.getRingIndex(_this.index);
								_this.innerRadius =
									_this.outerRadius - chart.radiusLength;
							...
							updateElement: function (arc, index, reset) {
								...
								if (!reset || !animationOpts.animateRotate) {
									...
									model.endAngle =
										model.startAngle + model.circumference;
								}
					...
					Chart.controllers.polarArea =
						Chart.DatasetController.extend({
							...
							update: function update(reset) {
								...
								_this.outerRadius =
									chart.outerRadius -
									chart.radiusLength * _this.index;
								_this.innerRadius =
									_this.outerRadius - chart.radiusLength;
							...
							helpers.each(
								...
								function (point, index) {
									...
									var controlPoints = helpers.splineCurve(
										...
										model,
										helpers.nextItem(meta.data, index, true)
											._model,
										model.tension,
									);
					...
					Chart.animationService = {
						...
						addAnimation: function (
							...
							for (
								...
								if (
									this.animations[index].chartInstance ===
									chartInstance
								) {
						...
						cancelAnimation: function (chartInstance) {
							var index = helpers.findIndex(
								...
								function (animationWrapper) {
									return (
										animationWrapper.chartInstance ===
										chartInstance
									);
								},
							);
						...
						startDigest: function () {
							...
							while (i < this.animations.length) {
								...
								if (
									...
									this.animations[i].chartInstance.animating =
										false;
							...
							var dropFrames =
								(endTime - startTime) / this.frameDuration;
				...
				module.exports = function (Chart) {
					...
					helpers.extend(Chart.Controller.prototype, {
						initialize: function initialize() {
							...
							Chart.pluginService.notifyPlugins("beforeInit", [
								this,
							]);
							...
							Chart.pluginService.notifyPlugins("afterInit", [
								this,
							]);
						...
						resize: function resize(silent) {
							...
							var newWidth = helpers.getMaximumWidth(
								this.chart.canvas,
							);
							var newHeight =
							...
							if (!silent) {
								this.stop();
								this.update(
									this.options.responsiveAnimationDuration,
								);
							}
						...
						buildScales: function buildScales() {
							...
							helpers.each(items, function (item, index) {
								...
								var scaleClass =
									Chart.scaleService.getScaleConstructor(
										scaleType,
									);
								if (!scaleClass) {
									return;
						...
						buildOrUpdateControllers:
							function buildOrUpdateControllers() {
								...
								helpers.each(
									...
									function (dataset, datasetIndex) {
										var meta =
											this.getDatasetMeta(datasetIndex);
										if (!meta.type) {
											meta.type =
												dataset.type ||
												this.config.type;
						...
						update: function update(animationDuration, lazy) {
							Chart.pluginService.notifyPlugins("beforeUpdate", [
								this,
							]);
							...
							Chart.pluginService.notifyPlugins(
								"afterScaleUpdate",
								[this],
							);
							...
							Chart.pluginService.notifyPlugins("afterUpdate", [
								this,
							]);
						...
						render: function render(duration, lazy) {
							Chart.pluginService.notifyPlugins("beforeRender", [
								this,
							]);
						...
						getElementAtEvent: function (e) {
							var eventPosition = helpers.getRelativePosition(
								e,
								this.chart,
							);
							var elementsArray = [];
						...
						destroy: function destroy() {
							...
							Chart.pluginService.notifyPlugins("destroy", [
								this,
							]);
						...
						eventHandler: function eventHandler(e) {
							...
							if (!me.animating) {
								...
								if (
									...
									me.render(
										hoverOptions.animationDuration,
										true,
									);
								}
							}
					...
					helpers.extend(Chart.DatasetController.prototype, {
						...
						addElements: function () {
							...
							for (i = 0, ilen = data.length; i < ilen; ++i) {
								metaData[i] =
									metaData[i] || me.createMetaData(meta, i);
							}
						...
						buildOrUpdateElements:
							function buildOrUpdateElements() {
				...
				module.exports = function (Chart) {
					...
					var easingEffects = (helpers.easingEffects = {
						...
						easeInOutBounce: function (t) {
							...
							return (
								easingEffects.easeOutBounce(t * 2 - 1) * 0.5 +
								1 * 0.5
							);
						},
					});
					...
					helpers.bindEvents = function (
						...
						var events = (chartInstance.events =
					...
					function getConstraintDimension(
						...
						var hasCContainer =
							isConstrainedValue(constrainedContainer);
					...
					helpers.removeResizeListener = function (node) {
						var hiddenIframe = node.querySelector(
				...
				module.exports = function () {
					...
					var Chart = function (context, config) {
						...
						this.width =
							...
							parseInt(
								Chart.helpers.getStyle(context.canvas, "width"),
							) ||
							Chart.helpers.getMaximumWidth(context.canvas);
						this.height =
							context.canvas.height ||
							parseInt(
								Chart.helpers.getStyle(
									context.canvas,
									"height",
								),
							) ||
				...
				module.exports = function (Chart) {
					...
					Chart.layoutService = {
						...
						removeBox: function (chartInstance, box) {
							...
							chartInstance.boxes.splice(
								chartInstance.boxes.indexOf(box),
								1,
							);
						},
					...
					Chart.Scale = Chart.Element.extend({
						...
						convertTicksToLabels: function () {
							...
							this.ticks = this.ticks.map(function (
								...
								return this.options.ticks.callback(
									numericalTick,
									index,
									ticks,
								);
						...
						calculateTickRotation: function () {
							...
							var firstWidth = context.measureText(
								this.ticks[0],
							).width;
						...
						fit: function () {
							...
							if (tickOpts.display && display) {
								...
								if (isHorizontal) {
									...
									this.paddingRight =
										this.labelRotation !== 0
											? sinRotation * (tickFontSize / 2) +
												3
											: lastLabelWidth / 2 + 3; // when rotated
								} else {
									// A vertical axis is more constrained by the width. Labels are the dominant factor here, so get that length first
									var maxLabelWidth =
										this.maxWidth - minSize.width;
						...
						getPixelForTick: function (index, includeOffset) {
							if (this.isHorizontal()) {
								...
								var pixel =
									tickWidth * index + this.paddingLeft;
								...
								return (
									this.top +
									index *
										(innerHeight / (this.ticks.length - 1))
								);
							}
						},
						...
						getPixelForDecimal: function (
							...
							if (this.isHorizontal()) {
								var innerWidth =
									...
									(this.paddingLeft + this.paddingRight);
								var valueOffset =
									innerWidth * decimal + this.paddingLeft;

								var finalVal =
									this.left + Math.round(valueOffset);
								finalVal += this.isFullWidth()
									? this.margins.left
									: 0;
								return finalVal;
						...
						draw: function (chartArea) {
							...
							var longestRotatedLabel =
								this.longestLabelWidth * cosRotation;
							...
							if (this.isHorizontal()) {
								...
								helpers.each(
									...
									function (label, index) {
										...
										var xLineValue =
											this.getPixelForTick(index); // xvalues for grid lines
										...
										if (optionTicks.display) {
											...
											context.rotate(
												labelRotationRadians * -1,
											);
											context.font = tickLabelFont;
											context.textAlign = isRotated
												? "right"
												: "center";
								...
								if (scaleLabel.display) {
									...
									scaleLabelX =
										this.left +
										(this.right - this.left) / 2; // midpoint of the width
									scaleLabelY =
										options.position === "bottom"
											? this.bottom -
												scaleLabelFontSize / 2
				...
				module.exports = function (Chart) {
					...
					Chart.Tooltip = Chart.Element.extend({
						...
						update: function (changed) {
							if (this._active.length) {
								...
								if (this._options.tooltips.mode === "single") {
									...
									tooltipPosition = this.getAveragePosition(
										this._active,
									);
								} else {
									helpers.each(
										...
										function (dataset, datasetIndex) {
											...
											var currentElement =
												meta.data[element._index];
											if (currentElement) {
												var yScale =
													element._yScale ||
													element._scale; // handle radar || polarArea charts
						...
						getTooltipSize: function getTooltipSize(vm) {
							...
							size.height +=
								(vm.title.length - 1) * vm.titleSpacing; // Title Line Spacing
							size.height += vm.title.length
							...
							helpers.each(
								vm.beforeBody.concat(vm.afterBody),
								function (line) {
									size.width = Math.max(
										size.width,
										ctx.measureText(line).width,
						...
						determineAlignment: function determineAlignment(size) {
							if (this._model.y < size.height) {
								...
								this._model.y >
								this._chart.height - size.height
							) {
							...
							if (this._model.yAlign === "center") {
								...
								rf = function (x) {
									return (
										x >= _this._chart.width - size.width / 2
									);
								};
							}
						...
						getBackgroundPoint: function getBackgroundPoint(
							vm,
							size,
						) {
							// Background Position
						...
						draw: function () {
							...
							function loopBackToStart(drawLineToCenter) {
								if (!first._view.skip && !last._view.skip) {
									...
									// Go to center
									ctx.lineTo(
										_this._view.scaleZero.x,
										_this._view.scaleZero.y,
									);
								}
							}
							...
							if (this._children.length > 0 && vm.fill) {
								...
								helpers.each(
									this._children,
									function (point, index) {
										var previous = helpers.previousItem(
											this._children,
											index,
										);
										var next = helpers.nextItem(
											this._children,
											index,
										);
					...
					var DatasetScale = Chart.Scale.extend({
						...
						getPixelForValue: function (
							...
							if (this.isHorizontal()) {
								...
								var widthOffset =
									valueWidth * (index - this.minIndex) +
									this.paddingLeft;
								...
								var heightOffset =
									valueHeight * (index - this.minIndex) +
									this.paddingTop;
				...
				module.exports = function (Chart) {
					...
					var LinearScale = Chart.Scale.extend({
							...
							if (opts.stacked) {
								...
								helpers.each(
									...
									function (valuesForType) {
										var values =
											valuesForType.positiveValues.concat(
										...
										_this.min =
											_this.min === null
												? minVal
												: Math.min(_this.min, minVal);
										_this.max =
											_this.max === null
												? maxVal
												: Math.max(_this.max, maxVal);
									},
								);
							} else {
								helpers.each(
						...
						buildTicks: function () {
							...
							var niceMin =
								Math.floor(_this.min / spacing) * spacing;
							var niceMax =
								Math.ceil(_this.max / spacing) * spacing;
						...
						getPixelForValue: function (
							...
							if (_this.isHorizontal()) {
								innerDimension =
									_this.width -
									(paddingLeft + _this.paddingRight);
								pixel =
									_this.left +
								...
								innerDimension =
									_this.height -
									(_this.paddingTop + paddingBottom);
								pixel =
									_this.bottom -
									paddingBottom -
									(innerDimension / range) *
						...
						getValueForPixel: function (pixel) {
							...
							var offset =
								(isHorizontal
									...
									: _this.bottom - paddingBottom - pixel) /
								innerDimension;
							return (
								_this.start + (_this.end - _this.start) * offset
							);
						},
						getPixelForTick: function (index, includeOffset) {
							return this.getPixelForValue(
				...
				module.exports = function (Chart) {
						...
						ticks: {
							callback: function (value, index, arr) {
								var remain =
									...
									Math.pow(
										10,
										Math.floor(helpers.log10(value)),
									);
					...
					var LogarithmicScale = Chart.Scale.extend({
						determineDataLimits: function () {
							...
							_this.min = getValueOrDefault(
								tickOpts.min,
								_this.min,
							);
							_this.max = getValueOrDefault(
								tickOpts.max,
								_this.max,
							);
							...
							if (_this.min === _this.max) {
								if (_this.min !== 0 && _this.min !== null) {
									_this.min = Math.pow(
										10,
										Math.floor(helpers.log10(_this.min)) -
											1,
									);
									_this.max = Math.pow(
										10,
										Math.floor(helpers.log10(_this.max)) +
											1,
									);
								} else {
									_this.min = 1;
						...
						buildTicks: function () {
							...
							while (tickVal < _this.max) {
								...
								var significand =
									Math.floor(tickVal / Math.pow(10, exp)) + 1;
						...
						getPixelForValue: function (
							...
							var range =
								helpers.log10(_this.end) - helpers.log10(start);
						...
						getValueForPixel: function (pixel) {
							...
							var offset;
							var range =
								helpers.log10(_this.end) -
								helpers.log10(_this.start);
							var value;
							var innerDimension;
							...
							if (_this.isHorizontal()) {
								innerDimension =
									_this.width -
									(_this.paddingLeft + _this.paddingRight);
								value =
									_this.start *
									Math.pow(
										10,
								...
								innerDimension =
									_this.height -
									(_this.paddingTop + _this.paddingBottom);
								value =
									Math.pow(
										10,
										((_this.bottom -
					...
					var LinearRadialScale = Chart.Scale.extend({
						...
						setDimensions: function () {
							...
							this.drawingArea = options.display
								? minSize / 2 -
									(tickFontSize / 2 +
										options.ticks.backdropPaddingY)
								: minSize / 2;
						},
						determineDataLimits: function () {
							this.min = null;
							...
							if (this.options.ticks.min !== undefined) {
								...
								this.options.ticks.suggestedMin !== undefined
							) {
								this.min = Math.min(
									this.min,
							...
							if (this.options.ticks.max !== undefined) {
								...
								this.options.ticks.suggestedMax !== undefined
							) {
								this.max = Math.max(
									this.max,
						...
						buildTicks: function () {
							...
							var spacing = helpers.niceNum(
								niceRange / (maxTicks - 1),
								true,
							);
							var niceMin =
								Math.floor(this.min / spacing) * spacing;
							var niceMax =
								Math.ceil(this.max / spacing) * spacing;

							var numSpaces = Math.ceil(
								(niceMax - niceMin) / spacing,
							);
						...
						fit: function () {
							...
							radiusReductionRight = helpers.isNumber(
								...
								? radiusReductionRight
								: 0;
							radiusReductionLeft = helpers.isNumber(
								radiusReductionLeft,
							)
								? radiusReductionLeft
								: 0;
							...
							if (this.options.display) {
								...
								if (!this.options.lineArc) {
									...
									for (
										...
										var pointLabeFontStyle =
											helpers.getValueOrDefault(
												this.options.pointLabels
													.fontStyle,
												globalDefaults.defaultFontStyle,
											);
										var pointLabeFontFamily =
											helpers.getValueOrDefault(
												this.options.pointLabels
													.fontFamily,
												globalDefaults.defaultFontFamily,
										...
										var labelsCount =
												this.pointLabels.length,
											halfLabelsCount =
												this.pointLabels.length / 2,
											quarterLabelsCount =
												halfLabelsCount / 2,
											upperHalf =
					...
					var TimeScale = Chart.Scale.extend({
						...
						determineDataLimits: function () {
							...
							if (
								...
								this.firstTick = moment.min.call(
									this,
									scaleLabelMoments,
								);
								this.lastTick = moment.max.call(
									this,
									scaleLabelMoments,
								);
							} else {
						...
						buildTicks: function (index) {
							...
							if (this.options.time.unit) {
								...
								var tickLabelWidth =
									this.ctx.measureText(tempFirstLabel).width;
								var cosRotation = Math.cos(
									helpers.toRadians(
										this.options.ticks.maxRotation,
									),
								);
								var sinRotation = Math.sin(
									helpers.toRadians(
								...
								var unitDefinition =
									time.units[unitDefinitionIndex];

								// While we aren't ideal and we don't have units left
								while (
									unitDefinitionIndex < time.units.length
								) {
							...
							if (!this.options.time.min) {
								...
								roundedStart = this.getMomentStartOf(
									this.firstTick,
								);
							}
							...
							if (!this.options.time.max) {
								var roundedEnd = this.getMomentStartOf(
									this.lastTick,
								);
								if (
									roundedEnd.diff(
							...
							helpers.each(
								...
								function (dataset, datasetIndex) {
									for (
										var i = 1;
										i <
										this.labelMoments[datasetIndex].length;
										i++
									) {
										this.smallestLabelSeparation = Math.min(
											this.smallestLabelSeparation,
											this.labelMoments[datasetIndex][
												i
											].diff(
												this.labelMoments[datasetIndex][
													i - 1
						...
						getLabelForIndex: function (index, datasetIndex) {
							var label =
								this.chart.data.labels &&
								index < this.chart.data.labels.length
									? this.chart.data.labels[index]
									: "";

							if (
								typeof this.chart.data.datasets[datasetIndex]
									.data[0] === "object"
							) {
						...
						getPixelForValue: function (
							...
							if (labelMoment) {
								...
								if (this.isHorizontal()) {
									...
									var valueWidth =
										innerWidth /
										Math.max(this.ticks.length - 1, 1);
									var valueOffset =
										innerWidth * decimal + this.paddingLeft;
									...
									var valueHeight =
										...
										Math.max(this.ticks.length - 1, 1);
									var heightOffset =
										innerHeight * decimal + this.paddingTop;
						...
						parseTime: function (label) {
							...
							if (
								typeof this.options.time.parser === "function"
							) {
