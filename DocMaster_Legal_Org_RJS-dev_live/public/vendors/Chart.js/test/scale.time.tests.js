...
describe("Time scale tests", function () {
	...
	it("should build ticks using date objects", function () {
		...
		function newDateFromRef(days) {
			return moment("01/01/2015 12:00", "DD/MM/YYYY HH:mm")
				.add(days, "d")
				.toDate();
		}
	...
	it("should allow custom time parsers", function () {
		...
		// Counts down because the lines are drawn top to bottom
		expect(xScale.ticks[0]).toEqualOneOf([
			"Nov 19, 1981",
			"Nov 20, 1981",
			"Nov 21, 1981",
		]); // handle time zone changes
		expect(xScale.ticks[1]).toEqualOneOf([
			"Nov 19, 1981",
			"Nov 20, 1981",
			"Nov 21, 1981",
		]); // handle time zone changes
	...
	it("should build ticks using the config unit", function () {
		...
		var mockContext = window.createMockContext();
		var config = Chart.helpers.clone(
