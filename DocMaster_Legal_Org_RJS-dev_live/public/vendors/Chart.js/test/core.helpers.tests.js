describe("Core helper tests", function () {
	...
	it("should merge scale configs", function () {
		...
		expect(merged.scales.yAxes[1].ticks.callback).toEqual(
			jasmine.any(Function),
		);
		expect(merged.scales.yAxes[2].ticks.callback).toEqual(
			jasmine.any(Function),
		);
	});
	...
	it("should get value or default", function () {
		...
		expect(helpers.getValueAtIndexOrDefault(undefined, undefined, 56)).toBe(
			56,
		);
	...
	it("should correctly determine if two numbers are essentially equal", function () {
		expect(
			helpers.almostEquals(0, Number.EPSILON, 2 * Number.EPSILON),
		).toBe(true);
		expect(helpers.almostEquals(1, 1.1, 0.0001)).toBe(false);
		expect(helpers.almostEquals(1e30, 1e30 + Number.EPSILON, 0)).toBe(
			false,
		);
		expect(
	...
	describe("Background hover color helper", function () {
		...
		it("should return a modified version of color when called with a color", function () {
			...
			expect(helpers.getHoverColor("#46BFBD")).not.toEqual(
				originalColorRGB,
			);
		});
	});
